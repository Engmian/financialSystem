...
import com.financial.entity.User_Login;
import com.financial.service.User_LoginService;
...
public class User_LoginController {
    ...
    public String getLogin(@RequestParam(value = "uPhone",required = false)String uPhone, HttpServletRequest request){
        String md5uPhone = MD5Utils.getMd5(uPhone);
        User_Login user_login = user_loginService.findUserList(md5uPhone);
        //如果登录失败则调回到登录页面，重新登录
        if (user_login == null) {
            return "login";
        }else {
